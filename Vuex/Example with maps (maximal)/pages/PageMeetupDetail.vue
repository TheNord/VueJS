...
<script>
  ...
  export default {
    computed: {
      ...mapState({
        // получаем текущий митап
        meetup: state => state.meetups.item,
        // получаем темы текущего митапа
    ...
    created () {
      ...
      // инициализурем текущий митап и его темы, передаем ид митапа
      this.fetchMeetupById(meetupId)
      this.fetchThreads(meetupId)
    },
    methods: {
      // вызываем экшен для получения темы и текущего митапа
